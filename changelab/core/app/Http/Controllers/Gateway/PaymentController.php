...
class PaymentController extends Controller
{
    ...
    public static function userDataUpdate($trx)
    {
        ...
        $exchange = Exchange::where('exchange_id', $trx)->where('deposit_id', $data->id)->first();
        if ($data->status == 0) {
            $data->status = 1;
            $data->save();

            $exchange->autometic_payment_status = 1;
            $exchange->save();
            ...
            notify($user, 'DEPOSIT_COMPLETE', [
                'method_name' => $data->exchange->payment_from_getway->name,
                'method_currency' => $gnl->cur_sym,
