...
<?php include('../inc/header1.php');?> 
	...
	$airtelvtu = $bil['airtelvtu'];
		...
		$per = $mtnvtu;	
			...
			$per = $airtelvtu;
				...
				$per = $etisalatvtu;
					...
					}elseif($network === 'glo' && $level !== $access){
		...
		$affpay = $afi['mtnvtu'];
			...
			$affpay = $afi['airtelvtu'];	
				}elseif($network === 'etisalat'){
				
				$affpay = $afi['9mobilevtu'];
...
$bilacc = "UPDATE users SET bal=bal-$debit WHERE email='$user'";
			$doDebit = $conn->query($bilacc);

include('../inc/payflex.php');
								
$qsel = "UPDATE transactions SET status='Completed',token='0',refer='N/A',channel='Wallet',amount='$xamount',charge='$debit' WHERE ref='$tref' ";	
$sav = $conn->query($qsel);
...
echo'<div class="alert alert-success">
										...
										<button type="button" class="close" data-dismiss="alert">×</button>
										<strong>Error Occur: Try again</strong>  
									</div>';  }
...
include('../inc/paystack-air.php');
 }else{
	 
include('../inc/flutterwave-air.php');	 
	 }
}
