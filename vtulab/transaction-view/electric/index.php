...
<?php include('../../inc/header2.php');?>
	...
	$ikeja = $rate['IkejaElectric'];
		...
		$product = "Ikeja Electricity Payment - IKEDC";
			
			}elseif($network === 'ibadan-electric'){
			
			$affpay = $afi['Ibedc'];
			
			$product = "Ibadan Electricity Payment - IBEDC";
				
			}elseif($network === 'kano-electric'){
					
					$affpay = $afi['Kedc'];
					
					$product = "Kano Electricity Payment -  KEDC";
				
						}elseif($network === 'jos-electric'){
							
							$affpay = $afi['JosElectric'];
							$product = "Jos Electricity Payment - JEDC";
							
							}elseif($network === 'eko-electric'){
								
								$affpay = $afi['EkoElectric'];
								$product = "Eko Electricity Payment - EKEDC";
								
								}elseif($network ==='portharcourt-electric'){
									
									$affpay = $afi['Phed'];
									$product = "Port Harcourt Electricity Payment - PHED";
	...
	$debit = $xamount-$comi;
			...
			$ret = mysql_query($conn,"SELECT * FROM transactions WHERE ref='$requestId' ");
		
		$da = mysql_fetch_array($ret);
		$tref = $da['ref'];
...
if(!empty($resp->description->Token)){
	...
	$qsel = "UPDATE transactions SET status='Completed',token='0',refer='N/A',channel='Wallet',amount='$xamount',charge='$chargeAmt' WHERE ref='$tref' ";	
$sav = $conn->query($qsel);
