...
if($chargeResponsecode === '00' && $paymentStatus === 'successful' ) {
    ...
    $payamt = $chargeAmount;
               ...
               $addfund = mysqli_query($conn, "UPDATE users SET bal=bal+$payamt WHERE email='$email'"); 
				...
				$up = "UPDATE transactions SET token='$tk',refer='$r',status='$statu',channel='Rave' WHERE ref='$tid' ";
				$conn->query($up);
...
$_SESSION['amtshow'] = $payamt;
          ...
          ?>
<script>window.location="buy_credit.php?ref='<?php echo $_GET['flwref']?>' ";</script> <?php    
